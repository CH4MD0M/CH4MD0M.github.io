...
**힙(heap)**은 <ins>완전 이진 트리의 일종</ins> 으로 특정한 규칙을 따라 부모 노드와 자식 노드 사이의 값이 정렬된다. 힙은 주로 **우선순위 큐(Priority Queue)**를 구현할 때 사용된다. 힙은 완전히 정렬된 것은 아니지만 전혀 정렬 되지 않은 상태도 아닌 <ins>반정렬 상태(느슨한 정렬 상태)를 유지한다.</ins>
...
자바스크립트로 코딩테스트 문제를 풀 때 힙을 직접 구현해서 풀어야'만' 해결할 수 있는 문제가 있는지는 모르겠다만 구현해놔서 손해보는 일은 없을거라고 생각한다!
...
>
> 만약, **MaxHeap을 사용하고 싶다면 데이터를 음수화해서 MinHeap을 사용하면 된다.**
