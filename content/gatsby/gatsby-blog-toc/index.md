...
> 이 부분에서 graphql로 불러올 수도 있었는데, 이 방법은 위에 설치한 `rehype-slug`와 함께 사용하기 힘든 방법이었다. 그래서 `document.querySelectorAll`을 사용하기로 했다. (이 부분이 틀렸다면 여러분 말이 무조건 맞다😅)
...
위 코드만으로도 ToC 생성과 제목 위치에 간다면 활성화되는 기능도 잘 작동한다. 하지만, 추가로 구현하고 싶었던 기능이 있었다. 매우 중요한 기능인지는 모르겠지만, 구현해보고 싶었던 기능이었다.😁

컨텐츠. 그러니까 제목 아래에 있는 글까지 포함해서 스크롤이 이동하면, ToC의 제목이 활성화되는 기능이다. 예를 들면, 2번 제목에서 스크롤 업할 때, 1번 제목의 글이 보인다면 1번 제목이 활성화되는 것이다.
